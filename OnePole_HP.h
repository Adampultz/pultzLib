...
public:
    ...
    OnePole_HP(double Fc, float sampleRate) {
...
inline void OnePole_HP::setFc(double Fc, float sampleRate) {
    double invSRate = 1.0 / sampleRate;
    b1 = -exp(-2.0 * M_PI * (0.5 - (Fc * invSRate)));
    a0 = 1.0 + b1;
}

inline float OnePole_HP::process(float s) {
