...
Biquad::Settings settings{
	...
	wavelets[n].setup(context->audioSampleRate, 0.9, 10, 0.01, 2, 1, 5);
...
void render(BelaContext *context, void *userData)
{
	...
	for(unsigned int n = 0; n < context->audioFrames; n++) {
		...
		for(unsigned int ch = 0; ch < gNumIns; ch++){
			...
			float blend = Balance2(dc[ch], stretch_[ch], balStretch);
		...
		if(gVarUpdateCount > gVarUpdateSens){
			...
			int	balance_ = (int) mapLin(clamp2(gSpectralBalance[i], 30.0f, 1500.0f) , 40.0f, 1000.0f, 8, 1);
			  ...
			  float rmsSlow = mapLin(rmsValSlow[i], 0, 1.0, 0.1, 0.9);
			  wavelets[i].setAnalysisWindow(clamp2(rmsSlow, 0.1f, 0.9f));
			  }
		}

 scope.log(rmsValSlow);
